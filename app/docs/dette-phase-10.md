...
| 11 | **Construire et démarrer la production fait partie de la vérification** | ~~Dette~~ **fermée le jour où elle a été ouverte.** Le build de production ne démarrait pas : `@thm/shared` exportait son source TypeScript, que `tsx` transpile à la volée en développement et que **rien** ne transpile en production. `lint`, `typecheck` et `test` passaient tous les trois sur un dépôt dont la production était cassée — parce qu'aucun d'eux ne construit, et qu'aucun ne lance l'artefact construit. La CI le fait maintenant. |
...
| 9 | **En-têtes de sécurité sur le FRONT** | `apps/api/src/http/securite.ts` ne couvre que l'API. Le front est servi en statique, par un hébergeur ou un proxy : c'est là que sa politique de contenu, son `X-Frame-Options` et son HSTS devront être posés. Aucun test ne peut l'attraper depuis ce dépôt. |
