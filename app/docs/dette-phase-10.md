...
| 6 bis | **Autoriser une ligne de requête d'au moins 8 ko sur le proxy** | `/api/rooms/batch` prend les codes en paramètres répétés. Mesuré le 2026-09-14 sur le dataset 1.0.0 : **4,0 ko** pour une tranche de 200 codes réels, **5,9 ko** dans le pire cas (les 200 codes les plus longs). Sous le défaut de nginx (`large_client_header_buffers`, 8 ko), mais un proxy réglé plus bas rendrait **414** sur la page `/progression` d'un utilisateur avancé, et sur lui seul. Aucun test ne peut attraper ça : la limite est dans la configuration du déploiement, pas dans le code. |
