...
| 6 ter | ~~**Trancher `TRUST_PROXY` au déploiement**~~ **CLOS le 2026-09-18, après être tombé exactement dans le piège décrit ici.** La mise en ligne du 17 septembre a posé `TRUST_PROXY=true`, la face « n'importe qui pose `X-Forwarded-For` ». Mesure sur le service en ligne le lendemain : trois requêtes nues font descendre le compteur de 119 à 117, trois requêtes portant chacune un `X-Forwarded-For` différent rendent **119, 119, 119** — trois compteurs neufs. La limite de débit ne limitait personne. **La cause racine était le TYPE, pas la valeur** : un booléen ne peut pas exprimer « un seul intermédiaire ». Corrigé en lisant `TRUST_PROXY` comme un **nombre d'intermédiaires**, et en refusant `true` au démarrage avec un message qui dit quoi mettre à la place. Un détour à signaler : le nombre que Fastify accepte pour `trustProxy` **ne compte pas les sauts** — mesuré, `1` et `2` ignorent l'en-tête et remettent tout le monde dans un seul seau, soit l'autre face du piège. Le comptage est donc fait par une fonction `(adresse, saut) => saut < n`. Trois tests le tiennent, dont un qui **reproduit la faille avec l'ancien réglage** pour prouver que les deux autres savent la voir. |
